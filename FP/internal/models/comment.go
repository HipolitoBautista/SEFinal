...
// Inserting all comments into a new row entry
func (m *CommentsModel) InsertComment(comments *Comments) error {

	//query with all comments to be inserted
	query := ` 
	...
	defer cancel()
	//query to be with the args to provide the query above with data
	...
	//Making sure the query executed properly
	if err != nil {
		return err
	}
	return nil
...
// Getting all the comments assocaited with the form
func (m *CommentsModel) GetComments(form_id int64) ([]*Comments, error) {
	//query which selects all comments assocaited with a specific form
	statement := `
		SELECT *
		FROM comments
		WHERE form_id = $1
	...
	//Getting every comment assocaited with that form
	rows, err := m.DB.Query(statement, form_id)
	if err != nil {
		//If no comments are found return empty comments
		...
		defer rows.Close()
		// pointer to every piece of data in the comments table
		commentsByte := []*Comments{}

		//parsing the only row
		rows.Next()
		//storing the comments returned by the query
		comment := &Comments{}
...
// checking to see if a comments table is already assocaited with comments ID
func (m *CommentsModel) Exists(form_id int64) bool {

	var count int
	//Query to see if the form already has comments assocaited with it
	query := ` 
	...
	//If count is 0 then it doesnt exist
	// if count is 1 it already exists
	if count == 0 {
		return false
	} else {
		return true
...
// Updating comments (instead of inserting because it already exists )
func (m *CommentsModel) UpdateComments(comments *Comments) (int64, error) {
	var id int64

	//Sql statement that will be ran to update the row
	statement := `
	UPDATE comments
